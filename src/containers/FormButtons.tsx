...
const FormButtons = ({ isLoading, isEdited, isDisabled, handleSave, handleCancel }: Props) => {
    return (
        <div className="flex absolute lg:bottom-10 bottom-5 right-5  gap-2">
            ...
            <button onClick={handleSave} type="submit" disabled={isDisabled} className={`${!isEdited && "opacity-70"} py-3 px-4 bg-[var(--save-btn-bg)] rounded-md text-white text-lg lg:text-xl enabled:hover:scale-105 enabled:hover:shadow-lg transition`}>
                {isLoading ?
                    <div className='ml-3'>
