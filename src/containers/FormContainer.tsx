...
const FormContainer = () => {
    ...
    const handleSave = (event: React.SyntheticEvent): void => {
        ...
        if (isEmpty(nameRef.current?.value) || isEmpty(phoneRef.current?.value) || !isValidEmail(emailRef.current?.value)) {
    ...
    return (
        <div className={`w-full md:w-1/2 p-5 pb-20 relative`}>
            <form onSubmit={handleSave} className='p-5 h-full overflow-y-auto'>
                {!isObjectEmpty(selectedUser) && <FormInputs handleInputChange={handleInputChange} nameRef={nameRef} phoneRef={phoneRef} emailRef={emailRef} userData={userData} />}
